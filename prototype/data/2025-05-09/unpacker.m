...
function [] = unpack2Dfig(figName)
    ...
    try
        ...
        datercell = [cell2mat(realXData(:).').', cell2mat(realYData(:).').', cell2mat(imagYData(:).').', cell2mat(magYData(:).').'];
        writematrix(datercell, fileName);
    end
end
...
function [] = unpack2DfigTime(figName)
    ...
    % Get the handle of the plot in the first subplot
    lineHandleReal = findobj(subplotHandleReal, 'Type', 'line');
    lineHandleImag = findobj(subplotHandleImag, 'Type', 'line');
    lineHandleMag = findobj(subplotHandleMag, 'Type', 'line');
...
function [time] = unpack2DfigTimeMaxTime(figName)
    fig = open(figName);
    ...
    % Get the x and y data of the plot
    realXData = get(lineHandleReal, 'XData');
    time = (max(realXData));
end